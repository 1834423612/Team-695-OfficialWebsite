    ...
    () => {
        ...
        const fetchUserInfo = async (forceRefresh = false) => {
            ...
            if (userInfo.value && !forceRefresh && now - lastUpdated.value < cacheDuration) {
                console.log('Using cached user info, last updated:', new Date(lastUpdated.value).toLocaleTimeString());
            ...
            try {
                ...
                console.log('User info updated at:', new Date(lastUpdated.value).toLocaleTimeString());
