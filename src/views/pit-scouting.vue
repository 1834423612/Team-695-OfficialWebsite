      ...
      <div class="px-4 py-5 sm:p-6">
        ...
        <form @submit.prevent="confirmSubmitForm">
          <div v-for="(field, index) in formFields" :key="index" class="mb-8 max-[640px]:mb-4 bg-gray-50 p-6 rounded-lg shadow-sm">
            <div v-if="field.type !== 'hidden'" class="mb-2">
              ...
              <div v-if="field.i" class="mt-4">
                <button @click="toggleDescription(index)" type="button"
                  ...
                  <span>{{ field.showDescription ? 'Hide' : 'Show' }} Description</span>
                  <Icon :icon="field.showDescription ? 'mdi:chevron-up' : 'mdi:chevron-down'" />
                </button>
                <transition name="fade">
                  <div v-if="field.showDescription" class="mt-2 bg-white p-4 rounded-md border border-blue-200">
                    <img :src="field.i" :alt="field.question" :style="{ width: field.w || '100%' }"
                      class="rounded-lg mb-4">
                    <p class="text-sm text-gray-600">{{ field.description || 'No additional description available.' }}
                    </p>
                  </div>
                </transition>
              </div>
              ...
              <div v-if="field.type === 'radio'" class="mt-4">
                <div v-for="(option, optionIndex) in field.options" :key="optionIndex" class="flex items-center mb-2">
                  <input :id="'field-' + index + '-' + optionIndex" :type="field.type" :name="'field-' + index"
                    :value="option" v-model="field.value" :required="field.required"
                    class="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300" @change="saveFormData">
                  <label :for="'field-' + index + '-' + optionIndex" class="ml-2 block text-sm text-gray-900">
                    {{ option }}
                  </label>
                </div>
                <div v-if="field.value === 'Other'" class="mt-2">
                  <input v-model="field.otherValue" type="text"
              ...
              <div v-else-if="field.type === 'checkbox'" class="mt-4">
                <div v-for="(option, optionIndex) in field.options" :key="optionIndex" class="flex items-center mb-2">
                  <input :id="'field-' + index + '-' + optionIndex" type="checkbox" :value="option"
                    v-model="field.value" class="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded"
                    @change="saveFormData">
                  <label :for="'field-' + index + '-' + optionIndex" class="ml-2 block text-sm text-gray-900">
                    {{ option }}
                  </label>
                </div>
                <div v-if="Array.isArray(field.value) && field.value.includes('Other')" class="mt-2">
                  <input v-model="field.otherValue" type="text"
                    :placeholder="'Specify other ' + field.question.toLowerCase()"
                    class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                    @input="saveFormData">
              ...
              <div v-else-if="field.type === 'autocomplete'" class="mt-1 relative">
                <input :id="'field-' + index" type="text" v-model="field.value" :required="field.required"
                  @input="searchTeams" @blur="hideTeamSuggestions"
                  class="block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                  :class="{ 'border-red-500': field.error }" :placeholder="'Enter team number'">
                <div v-if="showTeamSuggestions"
                  class="absolute z-10 mt-1 w-full bg-white shadow-lg max-h-60 rounded-md py-1 text-base ring-1 ring-black ring-opacity-5 overflow-auto focus:outline-none sm:text-sm">
                  <div v-for="team in teamSuggestions" :key="team.number" @mousedown="selectTeam(team)"
                    class="cursor-pointer select-none relative py-2 pl-3 pr-9 hover:bg-gray-100">
                    <div class="flex items-center">
                      <img :src="team.avatar" :alt="team.name" class="h-6 w-6 rounded-full mr-3">
                      <span class="font-normal truncate">{{ team.number }} - {{ team.name }}</span>
                    </div>
                  </div>
                </div>
                <p v-if="field.error" class="mt-2 text-sm text-red-600">{{ field.error }}</p>
              </div>

              <input v-else-if="field.type === 'text' || field.type === 'number'" :id="'field-' + index"
                :type="field.type" v-model="field.value" :required="field.required"
                class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                :class="{ 'border-red-500': field.error }" @input="saveFormData" @blur="validateField(field)">

              <textarea v-else-if="field.type === 'textarea'" :id="'field-' + index" v-model="field.value"
                :required="field.required" rows="3"
                class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                :class="{ 'border-red-500': field.error }" @input="saveFormData"
          ...
          <div class="mb-8 bg-gray-50 p-6 rounded-lg shadow-sm">
            <h2 class="text-xl font-semibold text-gray-900 mb-4">Full Robots Images</h2>
            <div @dragover.prevent @drop.prevent="handleDrop('fullRobot', $event)"
              ...
              <input type="file" accept="image/*" @change="handleFileSelect('fullRobot', $event)" class="hidden"
                ref="fullRobotInput" multiple>
              <button type="button" @click="$refs.fullRobotInput.click()"
            ...
            <div v-if="fullRobotImages.length > 0" class="mt-4 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
              <div v-for="(image, index) in fullRobotImages" :key="index"
                ...
                <button @click="confirmRemoveImage('fullRobot', index)"
          ...
          <div class="mb-8 bg-gray-50 p-6 rounded-lg shadow-sm">
            <h2 class="text-xl font-semibold text-gray-900 mb-4">Drive Train Images</h2>
            <div @dragover.prevent @drop.prevent="handleDrop('driveTrain', $event)"
              class="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center cursor-pointer hover:border-indigo-500 transition-colors duration-300">
              <input type="file" accept="image/*" @change="handleFileSelect('driveTrain', $event)" class="hidden"
                ref="driveTrainInput" multiple>
              <button type="button" @click="$refs.driveTrainInput.click()"
                class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                <Icon icon="mdi:upload" class="mr-2" />
                Select Files
              </button>
            ...
            <div v-if="driveTrainImages.length > 0" class="mt-4 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
              <div v-for="(image, index) in driveTrainImages" :key="index"
                class="relative bg-white p-2 rounded-lg shadow">
                <img :src="image.url" :alt="image.name" class="w-full h-32 object-cover rounded-lg">
                <div class="mt-2 text-xs text-gray-600 truncate">{{ image.name }}</div>
                <div class="text-xs text-gray-500">{{ formatFileSize(image.size) }}</div>
                <button @click="confirmRemoveImage('driveTrain', index)"
                  class="absolute top-1 right-1 bg-red-500 text-white rounded-full p-1 hover:bg-red-600 focus:outline-none">
                  <Icon icon="mdi:close" />
                </button>
              </div>
...
const tabs = ref<Tab[]>([{ name: 'User 1', formData: [], formId: uuidv4() }])
const currentTab = ref(0)
const eventId = ref('Event 123 - 2024')
...
const formFields = ref<FormField[]>([
  ...
  {
    question: "Robot Dimension (Length in Inches) without bumpers - front to back",
    type: "number",
    required: true,
    value: null
  },
  {
    question: "Robot Dimension (Width in Inches) without bumpers - left to right",
    type: "number",
    required: true,
    value: null
  },
  {
    question: "Robot Dimension (Height in Inches) from floor to highest point on robot at the start of the match",
    type: "number",
    required: true,
    value: null
  },
...
watch([tabs, currentTab], () => {
  saveToLocalStorage()
}, { deep: true })

const addTab = () => {
  const newTab: Tab = {
    name: `User ${tabs.value.length + 1}`,
    formData: JSON.parse(JSON.stringify(formFields.value.map(field => ({ ...field, value: null, error: undefined })))),
    formId: uuidv4()
  }
...
const clearCurrentTab = () => {
  tabs.value[currentTab.value].formData = JSON.parse(JSON.stringify(formFields.value.map(field => ({ ...field, value: null, error: undefined }))))
...
const toggleDescription = (index: number) => {
  formFields.value[index].showDescription = !formFields.value[index].showDescription
}
...
const selectTeam = (team: any) => {
  formFields.value[0].value = team.number
...
const handleFileSelect = async (type: 'fullRobot' | 'driveTrain', event: Event) => {
...
const handleDrop = async (type: 'fullRobot' | 'driveTrain', event: DragEvent) => {
...
const confirmRemoveImage = (type: 'fullRobot' | 'driveTrain', index: number) => {
  Swal.fire({
    title: 'Are you sure?',
    ...
    if (result.isConfirmed) {
      Swal.fire({
        title: 'Confirm Deletion',
        text: "Are you absolutely sure you want to delete this image?",
        icon: 'warning',
        showCancelButton: true,
        confirmButtonColor: '#d33',
        cancelButtonColor: '#3085d6',
...
const saveFormData = () => {
  ...
  localStorage.setItem(`formData_${currentFormId.value}`, JSON.stringify(formFields.value))
}

const saveImagesToLocalStorage = () => {
  localStorage.setItem(`fullRobotImages_${currentFormId.value}`, JSON.stringify(fullRobotImages.value))
  localStorage.setItem(`driveTrainImages_${currentFormId.value}`, JSON.stringify(driveTrainImages.value))
}

const loadImagesFromLocalStorage = () => {
  const savedFullRobotImages = localStorage.getItem(`fullRobotImages_${currentFormId.value}`)
  const savedDriveTrainImages = localStorage.getItem(`driveTrainImages_${currentFormId.value}`)
...
const validateField = (field: FormField) => {
  if (field.required) {
    if (field.type === 'radio' || field.type === 'checkbox') {
      ...
      if ((field.value === 'Other' || (Array.isArray(field.value) && field.value.includes('Other'))) && !field.otherValue) {
...
const submitForm = async () => {
  ...
  console.log('Full Robot Images:', fullRobotImages.value)
  console.log('Drive Train Images:', driveTrainImages.value)
  ...
  formFields.value = formFields.value.map(field => ({ ...field, value: null, error: undefined }))
  fullRobotImages.value = []
  driveTrainImages.value = []
